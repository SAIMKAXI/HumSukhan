...
const keys = () => [1, 2, 3].map((i) => Deno.env.get(`DEEPGRAM_API_KEY_${i}`)).filter((v): v is string => Boolean(v && v.trim()));
async function transcribe(key: string, audio: Uint8Array, mimeType: string, language: string, sampleRate: number, channels: number) {
  const params = new URLSearchParams({ model: "nova-3", smart_format: "true", punctuate: "true", language, encoding: "linear16", sample_rate: String(sampleRate), channels: String(channels) });
  const response = await fetch(`https://api.deepgram.com/v1/listen?${params}`, { method: "POST", headers: { Authorization: `Token ${key}`, "Content-Type": mimeType }, body: audio });
  if (!response.ok) return { ok: false, status: response.status, error: await response.text() };
  const payload = await response.json();
  const alternative = payload?.results?.channels?.[0]?.alternatives?.[0];
  return { ok: true, status: 200, transcript: alternative?.transcript ?? "", confidence: Number(alternative?.confidence ?? 0) };
}
Deno.serve(async (req) => {
  ...
  try {
    ...
    const candidates = requestedLanguage === "auto" || requestedLanguage === "multi" ? ["en", "ur"] : [requestedLanguage];
    ...
    for (const key of secretKeys) {
      ...
      for (const language of candidates) {
        ...
        if (!result.ok) { lastError = result.error; if ([401, 402, 403, 429].includes(result.status)) { keyRejected = true; break; } continue; }
        const transcript = result.transcript.trim();
        if (!transcript) continue;
        if (best == null || result.confidence > best.confidence) best = { transcript, confidence: result.confidence, language };
      }
      if (best != null) break;
      if (keyRejected) continue;
    }
    if (best != null) return Response.json({ transcript: best.transcript, provider: "deepgram", language: best.language, detectedLanguage: best.language, confidence: best.confidence }, { status: 200, headers: corsHeaders });
