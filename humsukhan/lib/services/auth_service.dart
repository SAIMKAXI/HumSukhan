...
class AuthService {
  ...
  static String? validatePassword(String password) {
    ...
    if (!_strongEightCharPassword.hasMatch(password)) return 'Password must contain uppercase, lowercase, number, and special character.';
    return null;
  }
  Future<AuthResult> signUp({required String email, required String password, String? name}) async {
    ...
    try {
      final response = await _supabase.auth!.signUp(email: email, password: password, data: name != null && name.isNotEmpty ? {'name': name} : null);
      final user = response.user;
      if (user == null) return AuthResult.failure('Account creation failed: no user returned.');
      await DatabaseService.instance.upsertProfile(UserProfile(id: user.id, name: name != null && name.isNotEmpty ? name : 'User'));
      return AuthResult.success(user);
    } on AuthException catch (e) { return AuthResult.failure(e.message); } catch (_) { return AuthResult.failure('Account created, but profile synchronization failed. Please try signing in again.'); }
  }
  Future<AuthResult> signIn({required String email, required String password}) async {
    if (!isAvailable) return AuthResult.failure('Authentication unavailable. Please check your connection and try again.');
    try { final response = await _supabase.auth!.signInWithPassword(email: email, password: password); if (response.user != null) return AuthResult.success(response.user!); return AuthResult.failure('Sign in failed: no user returned.'); } on AuthException catch (e) { return AuthResult.failure(e.message); } catch (_) { return AuthResult.failure('Sign in failed. Please try again.'); }
  }
  Future<AuthResult> signInAnonymously() async {
    if (!isAvailable) return AuthResult.failure('Authentication unavailable. Please check your connection and try again.');
    try { final response = await _supabase.auth!.signInAnonymously(); if (response.user != null) return AuthResult.success(response.user!); return AuthResult.failure('Anonymous sign in failed.'); } on AuthException catch (e) { return AuthResult.failure(e.message); } catch (_) { return AuthResult.failure('Anonymous sign in failed.'); }
  }
