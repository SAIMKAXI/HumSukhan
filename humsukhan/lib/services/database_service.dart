...
class DatabaseService {
  ...
  Future<void> upsertProfile(UserProfile profile) async {
    ...
    } catch (e) { debugPrint('Profile upsert error: $e'); }
  }

  Future<UserProfile?> fetchProfile(String userId) async {
    if (!_isAvailable) return null;
    try {
      final data = await _supabase.client!.from('profiles').select().eq('id', userId).maybeSingle();
    ...
    } catch (e) { debugPrint('Profile fetch error: $e'); return null; }
  }
  ...
  Future<void> upsertSettings(Map<String, dynamic> settings) async {
    ...
    try {
      ...
      await _supabase.client!.from('settings').upsert({'user_id': userId, 'settings': settings, 'updated_at': DateTime.now().toIso8601String()});
    } catch (e) { debugPrint('Settings upsert error: $e'); }
  }

  Future<Map<String, dynamic>?> fetchSettings(String userId) async {
    if (!_isAvailable) return null;
    try {
      final data = await _supabase.client!.from('settings').select().eq('user_id', userId).maybeSingle();
      if (data == null) return null;
      return Map<String, dynamic>.from(data['settings'] ?? {});
    } catch (e) { debugPrint('Settings fetch error: $e'); return null; }
  }
  ...
  Future<void> upsertSession(ProfessionalSession session) async {
    ...
    try {
      ...
      if (session.captions.isNotEmpty) {
        await _supabase.client!.from('captions').upsert(session.captions.map((c) => {'id': c.id, 'session_id': session.id, 'user_id': userId, 'text': c.text, 'speaker': c.speaker, 'timestamp': c.timestamp.toIso8601String(), 'language': c.language, 'is_partial': c.isPartial, 'is_own': c.isOwn}).toList());
      }
  ...
  Future<void> deleteSession(String sessionId) async {
    if (!_isAvailable) return;
    try {
      await _supabase.client!.from('captions').delete().eq('session_id', sessionId);
      await _supabase.client!.from('insights').delete().eq('session_id', sessionId);
      await _supabase.client!.from('sessions').delete().eq('id', sessionId);
    } catch (e) { debugPrint('Session delete error: $e'); }
  }

  Future<void> upsertFolder(Folder folder) async {
    if (!_isAvailable) return;
    try { await _supabase.client!.from('folders').upsert({'id': folder.id, 'user_id': _supabase.userId, 'name': folder.name, 'created_at': folder.createdAt.toIso8601String()}); } catch (e) { debugPrint('Folder upsert error: $e'); }
  }

  Future<List<Folder>> fetchFolders() async {
    if (!_isAvailable) return [];
    try {
      final data = await _supabase.client!.from('folders').select().eq('user_id', _supabase.userId).order('created_at');
      return data.map((row) => Folder(id: row['id'], name: row['name'] ?? '', createdAt: DateTime.parse(row['created_at']))).toList();
    } catch (e) { debugPrint('Folders fetch error: $e'); return []; }
  }

  Future<void> deleteFolder(String folderId) async {
    if (!_isAvailable) return;
    try { await _supabase.client!.from('folders').delete().eq('id', folderId); } catch (e) { debugPrint('Folder delete error: $e'); }
  }

  Future<void> upsertInsight(ProfessionalInsight insight) async {
    if (!_isAvailable) return;
    try { await _supabase.client!.from('insights').upsert({'id': insight.id, 'session_id': insight.sessionId, 'user_id': _supabase.userId, 'summary': insight.summary, 'vocabulary': insight.vocabulary, 'themes': insight.themes, 'action_items': insight.actionItems, 'deadlines': insight.deadlines, 'mentioned_people': insight.mentionedPeople, 'generated_at': insight.generatedAt.toIso8601String(), 'is_available': insight.isAvailable}); } catch (e) { debugPrint('Insight upsert error: $e'); }
  }
  ...
  Future<ProfessionalInsight?> fetchInsight(String sessionId) async {
    ...
    try {
      ...
      if (data == null) return null;
      return ProfessionalInsight(id: data['id'], sessionId: data['session_id'], summary: data['summary'] ?? '', vocabulary: List<String>.from(data['vocabulary'] ?? []), themes: List<String>.from(data['themes'] ?? []), actionItems: List<String>.from(data['action_items'] ?? []), deadlines: List<String>.from(data['deadlines'] ?? []), mentionedPeople: List<String>.from(data['mentioned_people'] ?? []), generatedAt: DateTime.parse(data['generated_at']), isAvailable: data['is_available'] ?? false);
  ...
  Future<void> upsertQuickReplies(List<QuickReply> replies) async {
    ...
    try {
      ...
      if (replies.isNotEmpty) await _supabase.client!.from('quick_replies').upsert(replies.map((r) => {'id': r.id, 'user_id': userId, 'text': r.text, 'category': r.category, 'is_favorite': r.isFavorite, 'created_at': r.createdAt.toIso8601String()}).toList());
    } catch (e) { debugPrint('Quick replies upsert error: $e'); }
  }

  Future<List<QuickReply>> fetchQuickReplies() async {
    if (!_isAvailable) return [];
    try {
      final data = await _supabase.client!.from('quick_replies').select().eq('user_id', _supabase.userId);
      return data.map((row) => QuickReply(id: row['id'], text: row['text'] ?? '', category: row['category'] ?? 'General', isFavorite: row['is_favorite'] ?? false, createdAt: DateTime.parse(row['created_at']))).toList();
    } catch (e) { debugPrint('Quick replies fetch error: $e'); return []; }
  }

  Future<int> cleanupExpiredSessions() async {
    if (!_isAvailable) return 0;
    try {
      final rows = await _supabase.client!.from('sessions').select('id').eq('user_id', _supabase.userId).lt('expires_at', DateTime.now().toIso8601String());
      for (final row in rows) await deleteSession(row['id']);
      return rows.length;
    } catch (e) { debugPrint('Cleanup error: $e'); return 0; }
  }
  ...
  Future<void> deleteAllUserData() async {
    ...
    } catch (e) { debugPrint('Delete all data error: $e'); }
  }
}
