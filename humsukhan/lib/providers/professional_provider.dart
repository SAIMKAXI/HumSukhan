...
class ProfessionalProvider extends ChangeNotifier {
  ...
  ProfessionalInsight _normalizeInsight(ProfessionalInsight insight){
    ...
    final summaryBullets=ActionItemNormalizer.removeActionOverlap(summaryBullets:rawSummaryBullets,actionItems:actionItems);
    ...
    final legacySummary=insight.summary.trim();
    final canonicalSummary=summaryBullets.isEmpty?legacySummary:summaryBullets.join(' ');
    return ProfessionalInsight(id:insight.id,sessionId:insight.sessionId,summary:canonicalSummary,summaryBullets:summaryBullets,actionItems:actionItems,deadlines:deadlines,mentionedPeople:mentionedPeople,generatedAt:insight.generatedAt,isAvailable:insight.isAvailable && canonicalSummary.isNotEmpty);
  }
  ...
  List<ProfessionalInsight> _canonicalizeInsights(Iterable<ProfessionalInsight> source){
    ...
    for(final raw in source){
      ...
      if(existing==null||insight.generatedAt.isAfter(existing.generatedAt))bySession[insight.sessionId]=insight;
    }
    final result=bySession.values.toList()..sort((a,b)=>b.generatedAt.compareTo(a.generatedAt));
    return result;
  }
  ...
  Future<ProfessionalSession> createSession({required String title,required SessionType type,String? folderId,String captionLanguage='English',int retentionDays=7}) async {final session=ProfessionalSession(title:title,type:type,folderId:folderId,captionLanguage:captionLanguage,retentionDays:retentionDays.clamp(1,15));_sessions.add(session);_activeSession=session;await _saveSessions();if(SupabaseService.instance.isAuthenticated)await DatabaseService.instance.upsertSession(session);notifyListeners();return session;}
  ...
  Future<void> deleteFolder(String folderId,{FolderDeleteMode mode=FolderDeleteMode.keepSessions}) async {await _persistQueue;final folderExists=_folders.any((f)=>f.id==folderId);if(!folderExists)return;final affected=_sessions.where((s)=>s.folderId==folderId).toList();if(mode==FolderDeleteMode.keepSessions){for(var i=0;i<_sessions.length;i++){if(_sessions[i].folderId==folderId)_sessions[i]=_sessions[i].copyWith(folderId:null);}if(SupabaseService.instance.isAuthenticated)await DatabaseService.instance.deleteFolder(folderId);}else{_sessions.removeWhere((s)=>s.folderId==folderId);_insights.removeWhere((i)=>affected.any((s)=>s.id==i.sessionId));if(SupabaseService.instance.isAuthenticated)await DatabaseService.instance.deleteFolderAndSessions(folderId);}_folders.removeWhere((f)=>f.id==folderId);await _saveFolders();await _saveSessions();await _saveInsights();if(mode==FolderDeleteMode.keepSessions&&SupabaseService.instance.isAuthenticated){for(final s in affected){await DatabaseService.instance.upsertSession(s.copyWith(folderId:null));}}notifyListeners();}
  ...
  Future<void> generateInsights(String sessionId) async {final session=_sessions.firstWhere((s)=>s.id==sessionId);final allText=session.captions.map((c)=>c.text).join('\n');if(allText.trim().isEmpty)return;final ai=AiService.instance.isAvailable?await AiService.instance.generateInsights(sessionId:sessionId,transcript:allText,sessionTitle:session.title,sessionType:session.type):null;if(ai==null||!ai.isAvailable)return;final normalized=_normalizeInsight(ai);if(!normalized.isAvailable||normalized.summaryBullets.isEmpty)return;_insights.removeWhere((i)=>i.sessionId==sessionId);_insights.add(normalized);await _saveInsights();if(SupabaseService.instance.isAuthenticated){await DatabaseService.instance.upsertInsight(normalized);}notifyListeners();}
