...
class ProfessionalProvider extends ChangeNotifier {
  ...
  Future<void> _syncFromCloud() async {
    try {
      final cloudSessions = await DatabaseService.instance.fetchSessions(); final localIds = _sessions.map((s)=>s.id).toSet();
      ...
      await _saveSessions();
      final cloudFolders = await DatabaseService.instance.fetchFolders(); final localFolderIds=_folders.map((f)=>f.id).toSet();
  ...
  Future<Folder> createFolder(String name) async { final folder=Folder(name:name.trim()); _folders.add(folder); await _saveFolders(); if(SupabaseService.instance.isAuthenticated)await DatabaseService.instance.upsertFolder(folder); notifyListeners(); return folder; }
  Future<void> deleteFolder(String folderId) async { await _persistQueue; final moved=<ProfessionalSession>[]; for(var i=0;i<_sessions.length;i++){if(_sessions[i].folderId==folderId){_sessions[i]=_sessions[i].copyWith(folderId:null);moved.add(_sessions[i]);}} if(SupabaseService.instance.isAuthenticated){await DatabaseService.instance.deleteFolder(folderId);} _folders.removeWhere((f)=>f.id==folderId); await _saveFolders(); await _saveSessions(); if(SupabaseService.instance.isAuthenticated){for(final s in moved){await DatabaseService.instance.upsertSession(s);}} notifyListeners(); }
  ...
  ProfessionalInsight _generateLocalInsights(ProfessionalSession session,String text){
    final sentences=text.split(RegExp(r'[.!?]+')).map((s)=>s.trim()).where((s)=>s.length>10).toList();
    final bullets=<String>[]; for(final sentence in sentences){if(bullets.length>=5)break;final clean='${sentence[0].toUpperCase()}${sentence.substring(1)}';if(!bullets.contains(clean))bullets.add(clean);}
    if(bullets.isEmpty)bullets.add('Session captured ${session.captions.length} caption${session.captions.length==1?'':'s'}.');
    final speakers=session.captions.map((c)=>c.speaker).toSet().where((s)=>s!='Speaker 1').toList();
    return ProfessionalInsight(sessionId:session.id,summary:bullets.join(' '),summaryBullets:bullets,actionItems:_extractActionItems(text),deadlines:_extractDeadlines(text),mentionedPeople:speakers,isAvailable:true);
  }
