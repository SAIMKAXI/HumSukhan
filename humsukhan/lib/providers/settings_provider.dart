...
class SettingsProvider extends ChangeNotifier {
  ...
  bool get isOnboardingComplete { final userId = AuthService.instance.currentUser?.id; return userId != null && _onboardedUsers.contains(userId); }
  ...
  Map<String, dynamic> _settingsPayload() => {
        'darkMode': _isDarkMode, 'highContrast': _isHighContrast, 'largeText': _isLargeText,
        'captionTextSize': _captionTextSize, 'hapticAlerts': _hapticAlerts, 'visualAlerts': _visualAlerts,
        'flashAlerts': _flashAlerts, 'screenFlashAlerts': _screenFlashAlerts,
        'captionLanguage': _captionLanguage, 'appLanguage': _appLanguage,
        'defaultRetentionDays': _defaultRetentionDays, 'monitoringEnabled': _monitoringEnabled,
  ...
  void _applySettings(Map<String, dynamic> data) {
    ...
    if (data['captionLanguage'] is String && const ['English', 'Roman Urdu', 'Urdu', 'Auto'].contains(data['captionLanguage'])) _captionLanguage = data['captionLanguage'] as String;
    ...
    if (alerts is Map) for (final entry in alerts.entries) { if (entry.key is String && entry.value is bool && _allowedAlerts.containsKey(entry.key)) _allowedAlerts[entry.key as String] = entry.value as bool; }
  }
  ...
  Future<void> _loadSettings() async {
    ...
    if (_legacyOnboardingComplete && currentUserId != null) { _onboardedUsers.add(currentUserId); await prefs.setBool('onboardingComplete:$currentUserId', true); }
    for (final key in prefs.getKeys()) { if (key.startsWith('onboardingComplete:') && prefs.getBool(key) == true) _onboardedUsers.add(key.substring('onboardingComplete:'.length)); }
    final storedAlerts = prefs.getString('allowedAlerts');
    if (storedAlerts != null) { try { _applySettings({'allowedAlerts': jsonDecode(storedAlerts)}); } catch (_) {} }
    _isLoaded = true; if (!_disposed) notifyListeners();
  ...
  Future<void> syncFromCloud() async {
    ...
    try {
      ...
      if (_disposed) return;
      if (cloudSettings != null && cloudSettings.isNotEmpty) { _applySettings(cloudSettings); await _persistAllLocal(); if (!_disposed) notifyListeners(); }
      else if (!_disposed) await _saveToCloud();
    } catch (e) { if (!_disposed) debugPrint('Settings cloud sync error: $e'); }
  }

  Future<void> _persistAllLocal([SharedPreferences? existing]) async { final prefs = existing ?? await SharedPreferences.getInstance(); if (_disposed) return; final data = _settingsPayload(); for (final entry in data.entries) { final value = entry.value; if (value is bool) await prefs.setBool(entry.key, value); if (value is double) await prefs.setDouble(entry.key, value); if (value is int) await prefs.setInt(entry.key, value); if (value is String) await prefs.setString(entry.key, value); if (entry.key == 'allowedAlerts') await prefs.setString(entry.key, jsonEncode(value)); if (_disposed) return; } }
  Future<void> _save(String key, dynamic value) async { final prefs = await SharedPreferences.getInstance(); if (_disposed) return; if (value is bool) await prefs.setBool(key, value); else if (value is double) await prefs.setDouble(key, value); else if (value is int) await prefs.setInt(key, value); else if (value is String) await prefs.setString(key, value); }
  Future<void> _saveToCloud() async { if (_disposed || !SupabaseService.instance.isAuthenticated) return; await DatabaseService.instance.upsertSettings(_settingsPayload()); }
  Future<void> _persistChange(String key, dynamic value) async { await _save(key, value); await _saveToCloud(); }
  ...
  Future<bool> hasCompletedOnboardingForUser(String userId) async { if (userId.isEmpty) return false; if (_onboardedUsers.contains(userId)) return true; final prefs = await SharedPreferences.getInstance(); final done = prefs.getBool('onboardingComplete:$userId') ?? false; if (done) _onboardedUsers.add(userId); return done; }
  Future<void> completeOnboardingForUser(String userId) async { if (userId.isEmpty) return; _onboardedUsers.add(userId); await _save('onboardingComplete:$userId', true); if (!_disposed) notifyListeners(); }
  Future<void> completeOnboarding() async { final userId = AuthService.instance.currentUser?.id; if (userId != null) await completeOnboardingForUser(userId); }
  Future<void> clearLocalSettings() async { final prefs = await SharedPreferences.getInstance(); if (_disposed) return; const keys = ['darkMode','highContrast','largeText','captionTextSize','hapticAlerts','visualAlerts','flashAlerts','screenFlashAlerts','captionLanguage','appLanguage','defaultRetentionDays','monitoringEnabled','allowedAlerts','onboardingComplete']; for (final key in keys) await prefs.remove(key); await prefs.remove('simplifiedLanguage'); final userKeys = prefs.getKeys().where((key) => key.startsWith('onboardingComplete:')).toList(); for (final key in userKeys) await prefs.remove(key); _isDarkMode=false; _isHighContrast=false; _isLargeText=false; _captionTextSize=24.0; _hapticAlerts=true; _visualAlerts=true; _flashAlerts=false; _screenFlashAlerts=true; _captionLanguage='English'; _appLanguage='en'; _defaultRetentionDays=7; _monitoringEnabled=false; _onboardedUsers.clear(); for (final key in _allowedAlerts.keys) _allowedAlerts[key]=true; if (!_disposed) notifyListeners(); }
