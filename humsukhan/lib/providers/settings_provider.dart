...
class SettingsProvider extends ChangeNotifier {
    ...
    if (storedAlerts != null) {
      try {
        ...
        for (final entry in decoded.entries) {
          _allowedAlerts[entry.key] = entry.value == true;
        }
      } catch (_) {
        // Ignore corrupt optional preference data and keep safe defaults.
      }
    }
    notifyListeners();
  }
  ...
  void setDefaultRetentionDays(int days) {
    _defaultRetentionDays = days.clamp(1, 15);
    _save('defaultRetentionDays', _defaultRetentionDays);
    notifyListeners();
  }
  ...
  void toggleAllowedAlert(String alertType) {
    ...
    _save('allowedAlerts', jsonEncode(_allowedAlerts));
    notifyListeners();
  }
