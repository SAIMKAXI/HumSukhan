...
class EnvironmentalMonitoringTileService : TileService() {
    override fun onStartListening() { super.onStartListening(); syncTile() }
    ...
    override fun onClick() {
        ...
        if (checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
            ...
            val pending = PendingIntent.getActivity(this, 4201, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) startActivityAndCollapse(pending) else {
    ...
    override fun onTileAdded() { super.onTileAdded(); syncTile() }
    ...
    companion object {
        fun requestUpdate(context: android.content.Context) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) TileService.requestListeningState(context, ComponentName(context, EnvironmentalMonitoringTileService::class.java))
        }
    }
}
